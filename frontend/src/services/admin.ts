...
export async function toggleUserBan(userId: string, isBanned: boolean) {
  return fetchWrapper<{ message: string; user: AdminUser }>(
...
export function getAdminTripDestinationLabel(trip: AdminTrip) {
  return formatTripDestination(
    trip.destinationPlace ?? trip.destination_place ?? trip.destination,
  );
}
...
export async function sendTripNotificationAsAdmin(
  tripId: string,
  payload: { type: string; message: string; broadcastToMembers: boolean },
) {
  return fetchWrapper<{ success: boolean; message: string; sentCount: number }>(
    `/admin/trips/${tripId}/notify`,
    {
      method: "POST",
...
export function getIdentityVerificationRequests(
  status?: IdentityVerificationStatus,
) {
