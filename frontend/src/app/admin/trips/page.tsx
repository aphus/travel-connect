...
import { Search, Eye, Bell, Loader2, Map, Calendar, Users, Filter, Navigation, CheckCircle2, Tent } from "lucide-react";
import { getAllTrips, sendTripNotificationAsAdmin, type AdminTrip } from "@/services/admin";
...
export default function AdminTripsPage() {
    ...
    const handleExecuteSendNotification = async (payload: { type: string, message: string, broadcastToMembers: boolean }) => {
        ...
        try {
            const response = await sendTripNotificationAsAdmin(selectedTrip.id, payload);
    ...
    return (
        <div className="p-6 space-y-8 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">Quản lý Chuyến đi</h1>
                    <p className="text-slate-500 mt-1.5 font-medium">Theo dõi tiến độ, tỉ lệ lấp đầy và hỗ trợ điều phối viên.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-100 transition-all group">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm font-semibold text-slate-500">Tổng Chuyến đi</p>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-amber-100 transition-all group">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm font-semibold text-slate-500">Đang diễn ra</p>
                            <p className="text-2xl font-black text-slate-800">{stats.ongoing}</p>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden relative">
                {isLoading && (
                    <div className="absolute inset-0 bg-white/60 backdrop-blur-sm z-10 flex flex-col items-center justify-center">
                        ...
                        <p className="font-semibold text-slate-600 animate-pulse">Đang đồng bộ dữ liệu...</p>
                    </div>
                )}
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredTrips.length > 0 ? (
                                filteredTrips.map((trip) => {
                                    ...
                                    const fillPercent = Math.min(Math.round((members / max) * 100), 100);
                                        ...
                                        <tr key={trip.id} className="hover:bg-slate-50/80 transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <span className="font-bold text-slate-800 text-base">{trip.destination}</span>
                                                        <p className="text-xs text-slate-400 font-mono mt-0.5">ID: {trip.id.substring(0, 8)}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2 text-slate-600 font-medium">
                                                    ...
                                                    {new Date(trip.startDate).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="w-full max-w-[140px]">
                                                    <div className="flex justify-between items-end mb-1.5">
                                                        <span className="text-xs font-bold text-slate-700">{members} / {max} <span className="text-slate-400 font-medium">người</span></span>
                                                        <span className="text-[10px] font-black text-slate-400">{fillPercent}%</span>
                                                    </div>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <span className={`inline-flex items-center justify-center px-3 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-wide border ${trip.status === 'completed' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                                                    trip.status === 'ongoing' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                                                        trip.status === 'upcoming' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <button
                                                        onClick={() => { setSelectedTrip(trip); setIsViewModalOpen(true); }}
                                                    ...
                                                    <button
                                                        onClick={(e) => {
                                                            ...
                                                            const statusLower = trip.status ? trip.status.toLowerCase() : '';
                                                            if (['completed', 'cancelled'].includes(statusLower)) {
                                                                alert("Chuyến đi đã kết thúc hoặc bị hủy, không thể gửi thông báo.");
                                                                return;
                                                            }
                                                            setSelectedTrip(trip);
                                                            setIsNotifyModalOpen(true);
                                                        }}
                                                        disabled={['completed', 'cancelled'].includes(trip.status ? trip.status.toLowerCase() : '')}
                                                        className={`p-2 transition-all rounded-lg ${['completed', 'cancelled'].includes(trip.status ? trip.status.toLowerCase() : '')
                                                                ? "text-slate-300 cursor-not-allowed opacity-50"
                                                                : "text-slate-400 hover:text-amber-600 hover:bg-amber-50"
                                                            }`}
                                                        title={['completed', 'cancelled'].includes(trip.status ? trip.status.toLowerCase() : '')
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-20 text-center">
                                        <div className="flex flex-col items-center justify-center text-slate-400">
                                            ...
                                            <p className="font-semibold text-slate-500">Không tìm thấy chuyến đi nào</p>
                                            <p className="text-sm mt-1">Thử thay đổi từ khóa hoặc bộ lọc trạng thái xem sao.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
